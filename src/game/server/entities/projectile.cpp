	...
	if( ((pTargetChr && (pOwnerChar ? !(pOwnerChar->m_Hit&CCharacter::DISABLE_HIT_GRENADE) : g_Config.m_SvHit || m_Owner == -1 || pTargetChr == pOwnerChar)) || Collide || GameLayerClipped(CurPos)) && !IsWeaponCollide)
	{
			...
			(m_Type == WEAPON_GRENADE && pOwnerChar->m_HasTeleGrenade) || (m_Type == WEAPON_GUN && pOwnerChar->m_HasTeleGun))
		{
			...
			if ((m_Type == WEAPON_GUN || m_Type == WEAPON_GRENADE) && (TileIndex != TILE_NO_TELE_GUN && TileFIndex != TILE_NO_TELE_GUN))
			{
				...
				vec2 PosInBlock = vec2(round_to_int(ColPos.x) % 32, round_to_int(ColPos.y) % 32);
				vec2 BlockCenter = vec2(round_to_int(ColPos.x), round_to_int(ColPos.y)) - PosInBlock + vec2(16.0f, 16.0f);
				GameServer()->CreateExplosion(BlockCenter, pOwnerChar->GetPlayer()->GetCID(), WEAPON_GRENADE, true, (m_Owner != -1) ? TeamMask : -1LL);
